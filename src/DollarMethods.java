...
public class DollarMethods 
{
	...
	public static ArrayList<Point> RotateBy(ArrayList<Point> points, double angle)
	{
		...
		for (Point p:points)
		{
			double qx = (p.x-c.x)*Math.cos(angle)-(p.y-c.y)*Math.sin(angle)+c.x;
			double qy = (p.x-c.x)*Math.sin(angle)+(p.y-c.y)*Math.cos(angle)+c.y;
			Point q  = new Point(qx,qy);
			newPoints.add(q);
		}
		return newPoints ;
	...
	public static ArrayList<Point> ScaleTo(ArrayList<Point> points, double s)
	{
		...
		for (Point p:points)
		{
			double qx = p.x*(s/(B[0]-B[1]));
			double qy = p.y*(s/(B[2]-B[3]));
			Point q  = new Point(qx,qy);
			newPoints.add(q);
		}
		return newPoints;
